...
private:
	DECL_HOOK_THISCALL(void, ClientModeShared__OverrideView, CViewSetup* renderView);
	...
	void OverrideView(CViewSetup* renderView);
...
void Camera::OverrideView(CViewSetup* renderView)
{
	...
	if (_y_spt_force_fov.GetBool())
		...
		current_cam.fov = renderView->fov;
	HandleDriveMode(control_type == 1);
	HandleCinematicMode(control_type == 2 && spt_demostuff.Demo_IsPlayingBack());
	if (control_type)
	{
		renderView->origin = current_cam.origin;
		renderView->angles = current_cam.angles;
	}
	renderView->fov = current_cam.fov;
}
...
HOOK_THISCALL(void, Camera, ClientModeShared__OverrideView, CViewSetup* renderView)
{
	spt_camera.ORIG_ClientModeShared__OverrideView(thisptr, edx, renderView);
	spt_camera.OverrideView(renderView);
}
