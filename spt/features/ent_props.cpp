...
CON_COMMAND(y_spt_print_ents, "Prints all entity indices and their corresponding classes.")
{
	...
	for (;;)
	{
		...
		int serverIdx = hasServerEnt ? (**serverIt).GetRefEHandle().GetEntryIndex() : INT_MAX;
		int clientIdx = hasClientEnt ? (**clientIt).GetRefEHandle().GetEntryIndex() : INT_MAX;
		const char* serverName = hasServerEnt ? utils::spt_serverEntList.EntClassName(*serverIt) : "<null>";
		serverName = serverName ? serverName : "<null>";
		const char* clientName = hasClientEnt ? utils::spt_clientEntList.EntClassName(*clientIt) : "<null>";
		clientName = clientName ? clientName : "<null>";

		if (serverIdx == clientIdx)
		{
			Msg("[%4d]; SERVER : %s, CLIENT : %s\n", serverIdx, serverName, clientName);
			++serverIt;
			++clientIt;
		}
		else if (serverIdx < clientIdx)
		{
...
CON_COMMAND(y_spt_print_ent_props, "Prints all props for a given entity index.")
{
	...
	{
		utils::PrintAllProps(atoi(args.Arg(1)));
	}
}
