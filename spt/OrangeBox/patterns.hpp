...
namespace patterns
{
	namespace engine
	{
		...
		PATTERNS(
		    CStaticPropMgr__DrawStaticProps,
		    "5135",
		    "56 6A 04 6A 00 68 ?? ?? ?? ?? 6A 00 8B F1 8B 0D ?? ?? ?? ?? 68 ?? ?? ?? ?? FF 15 ?? ?? ?? ?? A1");
		// these patterns are right after the first if which checks if the game is paused
		PATTERNS(
		    CDebugOverlay_AddTriangleOverlay,
		    "5135",
		    "56 57 FF 15 ?? ?? ?? ?? 8B C8 A1 ?? ?? ?? ?? 33 F6 3B C8 74 11 8B D1 BF ?? ?? ?? ?? 33 C0 F0 0F B1 17 85 C0 75 09 83 05 ?? ?? ?? ?? 01 EB 0F F3 90 56 51 B9 ?? ?? ?? ?? FF 15 ?? ?? ?? ?? 6A 4C E8 ?? ?? ?? ?? 83 C4 04 83 CF FF 3B C6 74 19");
		PATTERNS(
		    CDebugOverlay_AddLineOverlay,
		    "5135",
		    "56 57 FF 15 ?? ?? ?? ?? 8B C8 A1 ?? ?? ?? ?? 33 F6 3B C8 74 11 8B D1 BF ?? ?? ?? ?? 33 C0 F0 0F B1 17 85 C0 75 09 83 05 ?? ?? ?? ?? 01 EB 0F F3 90 56 51 B9 ?? ?? ?? ?? FF 15 ?? ?? ?? ?? 6A 40 E8 ?? ?? ?? ?? 83 C4 04 83 CF FF 3B C6 74 19");
	} // namespace engine
	...
	namespace client
	{
		...
		PATTERNS(CRendering3dView__DrawOpaqueRenderables,
		         "5135",
		         "55 8D 6C 24 8C 81 EC 94 00 00 00 53 56 57 6A 04 6A 00");
	} // namespace client
